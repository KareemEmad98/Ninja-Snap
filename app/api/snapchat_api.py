...
class SnapchatAPI:
    ...
    def add_users_to_segment(segment_id, schema_name, identifiers):
        ...
        payload = SnapchatAPI._prepare_add_users_to_segment_payload(
            segment_id, SnapchatAPI.SCHEMAS[schema_name], identifiers
        )
    ...
    def add_new_segment(segment_name, description, retention_days):
        session = SnapchatAPI.oauth_handler.get_session_with_headers()
        url = (
            f"https://adsapi.snapchat.com/v1/adaccounts/{Config.AD_ACCOUNT_ID}/segments"
        )
        payload = SnapchatAPI._prepare_add_segment_payload(
            segment_name, description, retention_days
        )
    ...
    def update_segment(segment_id, segment_name, description, retention_days):
        ...
        payload = SnapchatAPI._prepare_update_segment_payload(
            segment_id, segment_name, description, retention_days
        )
    ...
    @staticmethod
    def _prepare_update_segment_payload(
        segment_id, segment_name, description, retention_days
    ):
        return json.dumps(
            {
                ...
                "users": [
                    {
                        ...
                        "schema": [schema],
                        "data": [
                            [hashing.hash_identifier(identifier)]
    ...
    def _extract_segments_info_list(response_payload):
        return [
            ...
            for segment in response_payload["segments"]
        ]
