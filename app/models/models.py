...
class BaseModel(db.Model):
    ...
    def save(self):
        try:
            ...
            db_logger.error(f'Error saving {self.__class__.__name__}: {e}', exc_info=True)
            raise
    ...
    def delete(self):
        try:
            ...
            db_logger.error(f'Error deleting {self.__class__.__name__}: {e}', exc_info=True)
            raise
    ...
    def update(self, **kwargs):
        try:
            ...
            db.session.commit()
            db_logger.info(f'Updated {self.__class__.__name__} with ID {self.id}: {kwargs}')
        except Exception as e:
            db_logger.error(f'Error updating {self.__class__.__name__}: {e}', exc_info=True)
            raise
...
class Identifier(BaseModel):
    ...
    def save(self):
        existing_identifier = Identifier.query.filter_by(value=self.value, segment_id=self.segment_id).first()
