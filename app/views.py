...
def add_user_to_segment(segment_id):
    if request.method == 'POST':
        ...
        existing_identifiers = {id.value for id in Identifier.query.filter_by(segment_id=segment_id).all()}
        new_identifiers = [id_value for id_value in identifier_values if id_value not in existing_identifiers]
        errors = []
        if len(new_identifiers) < len(identifier_values):
            ...
            app_logger.warning(f"Duplicate identifiers not added: {duplicate_identifiers}")
        response = SnapchatAPI.add_users_to_segment(segment_id, identifier_type, identifier_values)
        app_logger.info(f"Response from Snapchat API: {response}")
        if response['request_status'] == 'SUCCESS':
            for id_value in new_identifiers:
                identifier_type = IdentifierType.query.filter_by(name=identifier_type).first()
                new_identifier = Identifier(value=id_value, type=identifier_type, segment_id=segment_id)
                new_identifier.save()
        ...
        if errors:
            ...
            return render_template('view_segment_users.html', segment=segment_id, identifiers=identifiers, errors=errors)
...
def add_segment():
    if request.method == 'POST':
        ...
        segment = SnapchatAPI.add_new_segment(segment_name, segment_description, segment_retention_days)
...
def update_segment(segment_id):
    if request.method == 'POST':
        ...
        segment_description = request.form['description']
        segment_retention_days = request.form['retention_days']
        SnapchatAPI.update_segment(segment_id, segment_name, segment_description, segment_retention_days)
...
def view_segment_identifiers(segment_id):
    ...
    identifiers = Identifier.query.filter_by(segment_id=segment_id).all()
    return render_template('view_segment_users.html', segment=segment_id, identifiers=identifiers)
